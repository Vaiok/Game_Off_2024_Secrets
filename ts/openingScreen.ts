...
class OpeningScreen {
    ...
    constructor(canvas: VisibleCanvas, sound: AudioContext) {
        this.canvas = canvas;
        this.sound = sound;
    }

    private drawOpeningScreen(selectedOption: OpeningScreenOptions) {
    ...
    private findSelectedOption(x: number, y: number, existingOption: OpeningScreenOptions[]): OpeningScreenOptions {
        const context = this.canvas.getContext();
        if (!context) { throw new Error('Could not get 2d context from canvas'); }
    ...
    private selectOptionMouse(event: MouseEvent, selectedOption: OpeningScreenOptions[]) {
        const canvas = this.canvas.getCanvas();
        const context = this.canvas.getContext();
        if (!context) { throw new Error('Could not get 2d context from canvas'); }
    ...
    private selectOptionKeyboard(event: KeyboardEvent, selectedOption: OpeningScreenOptions[], keyDown: boolean[]) {
        if (!keyDown[0]) {
            keyDown[0] = true;
    ...
    public runOpeningScreen() {
        ...
        window.removeEventListener('resize', () => this.drawOpeningScreen(selectedOption[0]));
        canvas.removeEventListener('click', (event) => this.selectOptionMouse(event, selectedOption));
        window.removeEventListener('keydown', (event) => this.selectOptionKeyboard(event, selectedOption, keyDown));
